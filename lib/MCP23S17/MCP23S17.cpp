/*
  ...
  January, 2015
  February, 2016
    CORRECTION Feb2017 by GCC [MCP::pinMode(unsigned int mode) had unneeded bit inversion]
  ...
  VERSION RELESE NOTES:
        V0.2
        Changed direct manipulation of pin 10 on ATMega168/328 via "PORTB" to use FdigitalWrite on an arbitrary
    ...
    :_SPIset(1000000, MSBFIRST, SPI_MODE0)
{
  ...
  //_SPIset = SPIsettings(1000000, MSBFIRST, SPI_MODE0);
};
...
unsigned int MCP::digitalRead(void) {       // This function will read all 16 bits of I/O, and return them as a word in the format 0x(portB)(portA)
  ...
  SPI.beginTransaction(_SPIset); // Start up the SPI bus
  ::FdigitalWrite(_ss, LOW);                 // Take slave-select low
  ...
  ::FdigitalWrite(_ss, HIGH);                // Take slave-select high
  SPI.endTransaction();
