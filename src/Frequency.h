...
#define IS_5DIGITS  (_BCDfrac == NO_FRAC)
...
class Frequency
{
    public:
        ...
        // 'trimmed' means that the leading '1' is not regarded
        uint8_t     setBCD(uint16_t f, uint8_t frac=NO_FRAC, char *ASCdst=NULL, uint8_t trimmed=0);
        uint8_t     setInt(uint16_t f, char *ASCdst, uint8_t trimmed=0);
        ...
        char        *ASC(char *ASCdst, uint8_t trimmed=0);
        ...
        void        addTicks(int8_t ticks, uint8_t isCOM, char *ASCdst, uint8_t trimmed=0);
        void        addkHz  (int8_t ticks, uint8_t isCOM, char *ASCdst, uint8_t trimmed=0);
        void        addMHz  (int8_t ticks, uint8_t isCOM, char *ASCdst, uint8_t trimmed=0);
...
class ADFFrequency
{
    public:
        ...
        uint8_t     setBCD(uint16_t fmain, uint16_t fext, char *ASCdst);
        uint8_t     setInt(uint16_t f, char *ASCdst);

        uint16_t    BCDmain(void)       { return _BCDfreqMain; }
        uint16_t    BCDext(void)        { return _BCDfreqExt;  }
        uint16_t    Int(void)           { return _Intfreq; }
        char        *ASC(char *ASCdst);

        uint8_t     check(int16_t f)    { return (f>=MINADF && f<=MAXADF);}
        uint8_t     addStep(int8_t ticks, uint8_t halfkHz=0, char *ASCdst=NULL);
