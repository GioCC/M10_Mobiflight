...
///
/// The base class Bank32 only consists of the "bare" flag vector;
/// the extended class Bank32c also manages flags to record bit change.

/// Warning: this class makes extensive use of virtual functions.
/// In order to spare memory on embedded systems (e.g. on gcc for AVR8 a class with 'n'
/// virtual functions seems to take up 4+(2*n) bytes for the virtual table),
/// a non-inheriting version of the classes is available. Of course, in this case
/// polymorphysm cannot be exploited.

#include "bitmasks.h"

//Read(byte pin, byte clean=1)
//ReadAll(byte clean=1)
...
class Bank32
{
    ...
    public:
        ...
        virtual uint32_t    write(byte pin, byte val)   { return (val ? lwset(data32.b32,pin-1) : lwclr(data32.b32,pin-1)); }
        virtual uint32_t    write(uint32_t vec)         { return (data32.b32 = vec); }
        virtual uint32_t    set(byte pin)               { return lwset(data32.b32,pin-1); }
        virtual uint32_t    set(void)                   { return (data32.b32 = 0xFFFFFFFF); }
        virtual uint32_t    clr(byte pin)               { return lwclr(data32.b32,pin-1); }
        virtual uint32_t    clr(void)                   { return (data32.b32 = 0x00000000); }

        // Lower word: pin=1..16
        uint16_t            valL(byte pin)              { return wbit(data32.b16.low,pin-1); } // & (clean ? IOcfg.b16.low : 0xFFFF)); }
        uint16_t            valL(void)                  { return data32.b16.low; } // & (clean ? IOcfg.b16.low : 0xFFFF)); }
        virtual uint16_t    writeL(byte pin, byte val)  { return (val ? wset(data32.b16.low,pin-1) : wclr(data32.b16.low,pin-1)); }
        virtual uint16_t    writeL(uint16_t vec)        { return (data32.b16.low = vec); }
        virtual uint16_t    setL(byte pin)              { return wset(data32.b16.low,pin-1); }
        virtual uint16_t    setL(void)                  { return (data32.b16.low = 0xFFFF); }
        virtual uint16_t    clrL(byte pin)              { return wclr(data32.b16.low,pin-1); }
        virtual uint16_t    clrL(void)                  { return (data32.b16.low = 0x0000); }

        // Upper word: pin=1..16 (maps to 17..32 of LongWord)
        uint16_t            valH(byte pin)             { return wbit(data32.b16.high,pin-1); } //  & (clean ? IOcfg.b16.high : 0xFFFF)); }
        uint16_t            valH(void)                 { return data32.b16.high; } //  & (clean ? IOcfg.b16.high : 0xFFFF)); }
        virtual uint16_t    writeH(byte pin, byte val)  { return (val ? wset(data32.b16.high,pin-1) : wclr(data32.b16.high,pin-1)); }
        virtual uint16_t    writeH(uint16_t vec)        { return (data32.b16.high = vec); }
        virtual uint16_t    setH(byte pin)              { return wset(data32.b16.high,pin-1); }
        virtual uint16_t    setH(void)                  { return (data32.b16.high = 0xFFFF); }
        virtual uint16_t    clrH(byte pin)              { return wclr(data32.b16.high,pin-1); }
        virtual uint16_t    clrH(void)                  { return (data32.b16.high = 0x0000); }
};
